...
<div class="margem">
    <div id="atualizarPlanos">
        ...
        if (isset($mensagem)):
            ...
            <?php elseif ($mensagem == "Problema ao atualizar!"): ?>
    </div>
...
<form id="formPlanos" method="Post" action="<?php echo BASE_URL; ?>Planos/atualizar">
    ...
    <select class="campoTexto" name="parcela">
        ...
        <?php
        for ($i = 1; $i <= 48; $i++) {
            echo '<option value="' . $i . '" ' . ($planos['parcela'] == $i ? 'selected' : '') . '>' . $i . 'x</option>';
        }
    ...
    <input id="indenizacao" class="campoTexto" type="text" name="indenizacao" value="<?php echo $planos['indenizacao']; ?>" />
    <script>
    ...
    ?>
        ...
        ?>
            ...
            <input id="<?php echo $complementPlan['idComplementoPlano']; ?>" type="radio" name="comSemSeguro[]" <?php if (isset($todosMatchPlanosArray)) {
                                                                                                                    echo $complementPlan['idComplementoPlano'] == $todosMatchPlanosArray[0]['complementoPlano_idComplementoPlano'] ? 'checked' : '';
                                                                                                                } ?> value="<?php echo $complementPlan['idComplementoPlano']; ?>">
